...
import { Sparkles, Shield, Clock, CreditCard, ArrowRight, MapPin, Star, Bus } from 'lucide-react'
...
const features = [
  ...
  { icon: <Clock className="h-6 w-6" />, title: 'Real-time Updates', description: 'Live availability and schedule updates' },
  { icon: <CreditCard className="h-6 w-6" />, title: 'Easy Payments', description: 'Multiple payment options including M-Pesa' }
]
      ...
      <section className="relative overflow-hidden bg-hero-pattern py-20 md:py-32">
        ...
        <div className="container mx-auto px-4 relative z-10">
          <motion.div initial={{ opacity: 0, y: 30 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.6 }} className="text-center mb-12">
            ...
            <h1 className="font-display text-4xl md:text-6xl lg:text-7xl font-bold mb-6"><span className="text-foreground">Travel Across</span><br /><span className="text-gradient">Kenya with Ease</span></h1>
            <p className="text-lg md:text-xl text-muted-foreground max-w-2xl mx-auto mb-8">Book buses, discover routes, and travel smarter with our AI-powered platform.</p>
          </motion.div>
      ...
      <section className="py-20 bg-card/50">
        <div className="container mx-auto px-4">
          <motion.div initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} className="text-center mb-12">
            ...
            <p className="text-muted-foreground max-w-xl mx-auto">Experience the future of travel booking</p>
          </motion.div>
      ...
      <section className="py-20">
        <div className="container mx-auto px-4">
          <motion.div initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} className="flex flex-col md:flex-row justify-between items-start md:items-center mb-12 gap-4">
            <div><h2 className="font-display text-3xl md:text-4xl font-bold mb-2">Popular <span className="text-gradient">Destinations</span></h2><p className="text-muted-foreground">Explore Kenya's most traveled routes</p></div>
            <Button variant="outline" onClick={() => navigate('/search')}>View All Routes<ArrowRight className="ml-2 h-4 w-4" /></Button>
          </motion.div>
          <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-4">
            {destinations.map((city, index) => (<motion.button key={city} initial={{ opacity: 0, scale: 0.9 }} whileInView={{ opacity: 1, scale: 1 }} viewport={{ once: true }} transition={{ delay: index * 0.05 }} onClick={() => navigate(`/search?destination=${city}`)} className="group relative overflow-hidden rounded-xl aspect-square"><img src={`https://source.unsplash.com/400x400/?${city},kenya,city`} alt={city} className="w-full h-full object-cover transition-transform duration-500 group-hover:scale-110" /><div className="absolute inset-0 bg-gradient-to-t from-black/80 via-black/20 to-transparent" /><div className="absolute bottom-0 left-0 right-0 p-4"><div className="flex items-center gap-2 text-white"><MapPin className="h-4 w-4" /><span className="font-medium">{city}</span></div></div></motion.button>))}
          </div>
        </div>
      </section>
        ...
        <div className="container mx-auto px-4">
          <motion.div initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} className="relative overflow-hidden rounded-3xl bg-gradient-to-br from-primary via-orange-500 to-accent p-8 md:p-16 text-center">
            ...
            <div className="relative z-10">
              ...
              <h2 className="font-display text-3xl md:text-5xl font-bold text-white mb-4">Ready to Start Your Journey?</h2>
              <p className="text-white/80 text-lg max-w-xl mx-auto mb-8">Join thousands of travelers who trust SafariRides.</p>
