...
import { Filter, SlidersHorizontal, X, Sparkles, Search as SearchIcon } from 'lucide-react'
...
import type { Trip, SearchParams, AIRecommendationResponse } from '@/types'
...
export default function SearchPage() {
  ...
  const loadTrips = useCallback(async () => {
    ...
    try {
      ...
      if (aiQuery) {
        ...
        const params: SearchParams = { origin: searchParams.get('origin') || undefined, destination: searchParams.get('destination') || undefined, date: searchParams.get('date') || undefined, min_price: priceRange[0] > 0 ? priceRange[0] : undefined, max_price: priceRange[1] < 5000 ? priceRange[1] : undefined, sort_by: sortBy as SearchParams['sort_by'], sort_order: sortOrder as SearchParams['sort_order'], page: parseInt(searchParams.get('page') || '1'), limit: 10 }
  ...
  }, [searchParams, priceRange, sortBy, sortOrder])
  ...
  const clearFilters = () => { setPriceRange([0, 5000]); setSortBy('departure_time'); setSortOrder('asc'); setSearchParams({}); setAiExplanation(null) }
  const hasActiveFilters = searchParams.get('origin') || searchParams.get('destination') || searchParams.get('date') || priceRange[0] > 0 || priceRange[1] < 5000
